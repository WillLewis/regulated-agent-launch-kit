"""Build the credential-free replay fixture for the semantic regression seeds.

The 3 adversarial v1 semantic-only regression seeds were detectable only by the
model/NLI semantic grader. This script makes them **replayable without
credentials** by emitting a ``SemanticDecision`` fixture — the same precomputed-
decision contract the existing fixture lane uses (``run_eval.py
--semantic-decisions``), so the pure offline grader fires
``UNSAFE_CUSTOMER_COMMS`` on the seeded cases with **no model call**.

What is pinned, and from where:

- ``makes_unsupported_claim: true`` — taken from the public semantic audit
  summary's ``semantic_only_flag_case_ids`` (the sanitized aggregate verdict).
  The script refuses to emit a decision for any seed whose
  ``(source_case_id, source_agent_system_version)`` is not a semantic-only flag
  in that summary, so the fixture cannot drift from the audit.
...
- ``claim_type`` / ``calibration`` — intentionally **not** pinned per case
  (set to ``none`` / ``unknown``): the public summary exposes only per-profile
  histograms, not a per-case mapping, and the grader fires on
  ``makes_unsupported_claim`` alone. No raw decision payloads are read.
...
The fixture is keyed by the **deterministic replay profile** (default
``improved_v0``) — a credential-free vehicle to exercise the offline grader
pipeline. It does not re-derive the claim from a live draft; that would need
credentials. Evaluator/grader separation is unaffected: this only feeds the
offline ``unsupported_claim_semantic`` grader, never the runtime EvaluatorNode.
"""
...
def _load_summary(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise SystemExit(
            ...
            "  Hint: run `make semantic-audit-summary-adversarial-v1-llm` (on-disk)."
        )
...
def build_fixture(
    *,
    ...
    for record in records:
        ...
        decisions[regression_case_id] = {
            ...
            "rationale": (
                ...
                f"{source_profile}, per the public semantic audit summary (the "
    ...
    return {
        ...
        "note": (
            "Credential-free replay fixture for the adversarial v1 semantic "
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "adversarial v1 semantic regression seeds. On-disk only; no model call."
        )
    )
