"""Build a credential-free replay fixture from a v2 candidate model/NLI audit.
    ...
    *,
    ...
    for case_id, decision in src.items():
        ...
        replay[str(case_id)] = {
            ...
            "rationale": (
                f"{RATIONALE_PREFIX} for adversarial v2 candidate profile "
    ...
    fixture = {
        ...
        "note": (
            ...
            f"adversarial v2 {candidate_profile} drafts under the deterministic "
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Re-key a raw adversarial v2 candidate model/NLI decision file into a "
    ...
    if not args.decisions.exists():
        raise SystemExit(
            ...
            "them. Hint: run `make semantic-model-decisions-adversarial-v2-llm-v1` "
            "(credentialed) first."
        )
