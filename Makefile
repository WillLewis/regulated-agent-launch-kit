...
# Credential-free: calibrate the characterized consent over-flag
# (supported_consent_fact_overflagged) out of the v2.3 held-out gate, then
# re-gate. Reads on-disk raw decisions; NO model call. Clears a flag ONLY when
# the synthetic consent_state is granted (state-supported). The clearance log
# reports/llm_adversarial_v3_candidate_v2_3_consent_calibration.{json,md} is
# public-safe and tracked. M7 stays OPEN / NOT READY FOR PILOT.
semantic-gate-adversarial-v3-llm-v2-3-calibrated:
	...
	uv run python scripts/apply_semantic_consent_calibration.py \
		--decisions reports/semantic_model_decisions/adversarial_v3_llm_candidate_v2_3.json \
		--dataset case_studies/financial_links_reliability/evals/adversarial_v3.jsonl \
		--out reports/semantic_model_decisions/adversarial_v3_llm_candidate_v2_3_calibrated.json \
		--log-out reports/llm_adversarial_v3_candidate_v2_3_consent_calibration.json
	uv run python scripts/build_semantic_replay_adversarial_v3_llm.py \
		--decisions reports/semantic_model_decisions/adversarial_v3_llm_candidate_v2_3_calibrated.json \
		--out reports/llm_adversarial_v3_candidate_v2_3_calibrated_semantic_replay_decisions.json
	uv run python scripts/run_eval.py \
		--dataset case_studies/financial_links_reliability/evals/adversarial_v3.jsonl \
		--traces-out traces/local/llm_adversarial_v3_candidate_v2_3_calibrated_semantic_model \
		--report-out reports/llm_adversarial_v3_candidate_v2_3_calibrated_semantic_model_eval.json \
		--agent-system-version improved_v0 \
		--semantic-decisions reports/llm_adversarial_v3_candidate_v2_3_calibrated_semantic_replay_decisions.json
	uv run python scripts/check_semantic_gate.py \
		--report reports/llm_adversarial_v3_candidate_v2_3_calibrated_semantic_model_eval.json
...
# ---- Opt-in credentialed repeat-run capture — M7e v2.3 on held-out v3 -------
# Runs the full v2.3/v3 variance loop:
#   1. credentialed v2.3 draft eval on adversarial_v3;
#   2. credential-free deterministic forward-looking audit;
#   3. credentialed model/NLI semantic decisions;
#   4. credential-free consent calibration + semantic gate replay;
#   5. public-safe variance summary at reports/..._variance_summary.{md,json}.
# Raw per-run reports, traces, and model decisions stay under reports/llm_repeats/
# and remain gitignored. Default RUNS=5.

repeat-v2-3-v3: check-llm-env
	uv run python scripts/run_v2_3_v3_variance.py \
		--runs $(RUNS) \
		--summary-md reports/llm_adversarial_v3_candidate_v2_3_variance_summary.md \
		--summary-json reports/llm_adversarial_v3_candidate_v2_3_variance_summary.json
