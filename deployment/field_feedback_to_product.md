...
| F5 | The live FL loop is `draft_only`, but the human-approval *suspension primitive* now exists and is proven on a synthetic harness (M9). | `app/action_suspension.py` (real LangGraph interrupt before `HumanApprovalNode`) + `traces/local/action_suspension/`; live loop still `configs/approval_matrix.yaml` (`draft_only`) | SDK or tool integration gap | Residual ask: wire the proven M9 suspension primitive into a real partner tool call beyond `draft_only`, so approval gates an actual (not synthetic) action end-to-end. |
